        ...
        for (int t = 0; t < tryCount; t++) {
            ...
            while (true) {
                ...
                for (int idx = 0; idx < combiCnt; idx++) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        if (Predict[i] == allCombi[idx][i]) tempStrike++;
                        else if (Predict[i] == allCombi[idx][0] || Predict[i] == allCombi[idx][1]|| Predict[i] == allCombi[idx][2]) tempBall++;
                    }
                    if (tempStrike == strike && tempBall == ball) {
                        allCombi[newCnt][0] = allCombi[idx][0];
                        allCombi[newCnt][1] = allCombi[idx][1];
                        allCombi[newCnt][2] = allCombi[idx][2];
                        newCnt++;
                    }
                }
                combiCnt = newCnt;
